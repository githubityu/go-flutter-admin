...
func Routers() *gin.Engine {
	...
	//Router.Static("/static/", "./resource/upload")
	Router.StaticFS("/static/", http.Dir("resource/upload"))
	// 跨域，如需跨域可以打开下面的注释
	...
	baseRouter := router.RouterGroupApp.Base
	{
		...
		cmsRouter.InitArticleRouter(PublicGroup)
		cmsRouter.InitChannelRouter(PublicGroup)
	}
	...
	PrivateGroup.Use(middleware.JWTAuth()).Use(middleware.CasbinHandler())
	{
		...
		authorityRouter.InitSysDictionaryRouter(PrivateGroup)       // 接口
		authorityRouter.InitSysDictionaryDetailRouter(PrivateGroup) // 接口
		// 系统监控
		monitorRouter.InitOperationLogRouter(PrivateGroup) // 操作日志
		// 文件管理
		fileMRouter.InitFileRouter(PublicGroup)
		// 系统工具
		sysToolRouter.InitCronRouter(PrivateGroup)
		//cmsRouter.InitRouter(PrivateGroup)
	}
